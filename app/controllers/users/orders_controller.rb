...
class Users::OrdersController < ApplicationController
  ...
  def deliver
    redirect_to users_orders_url, alert: 'You are not authorized to perform this action, please ask to admin to update status' and return unless current_user.role.name == 'Admin' || current_user.role.name == 'Manager'
  ...
  def order_params
    params.require(:order).permit(:status, :price, :tax, :discount, :total_price, :description, :file, :transaction_id, :eta, :trackingurl, :payment_method, order_products_attributes: %i[id product_id quantity price final_price _destroy])
  end
end
